...
export class ManageLocationComponent implements OnInit, OnDestroy {
  ...
  constructor(private apiService: vaplongapi, private notificationService: NotificationService, private storageService: StorageService,public router: Router) {
 ...
 GoToLifelineReport(){
  ...
  validationForNonTrackable(selectedLocationID) {
    // tslint:disable-next-line: deprecation
    if (this.selectedLevelID === '' || isNullOrUndefined(this.selectedLevelID)) {
      this.notificationService.notify(NotificationEnum.ERROR, 'Error', 'Please select level');
      return false;
    ...
    const selected = this.AllNonTrackableProductsLocationList.filter(x =>
