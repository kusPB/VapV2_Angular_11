...
@Component({
  selector: 'app-stock-index-prices',
  templateUrl: './stock-index-prices.component.html',
  styleUrls: ['./stock-index-prices.component.scss'],
  providers: [DatePipe]

})
export class StockIndexPricesComponent implements OnInit, OnDestroy {
  AllStockList: Stock[] = [];
  selectedStock: Stock;
  ...
  columns: Columns[] = [
    ...
    { field: 'RemainingStock', header: 'Available Stock', sorting: 'RemainingStock', placeholder: '', searching: true, translateCol: 'SSGENERIC.AVAILABLE' },
    {
      field: 'AvgPurchasePrice', header: 'Avg. Purchase Price(last 3 Purchases)', sorting: 'AvgPurchasePrice', placeholder: '',
      type: TableColumnEnum.CURRENCY_SYMBOL, searching: true,
      translateCol: 'SSGENERIC.AVGPURCHASE'
    },
    {
      field: 'PurchasePrice', header: 'Purchase Price', sorting: 'PurchasePrice', placeholder: '',
      type: TableColumnEnum.CURRENCY_SYMBOL, searching: true, translateCol: 'SSGENERIC.PURCHASEP'
    }
  ];



  globalFilterFields = ['Product', 'ProductVariantID', 'ProductModel', 'Barcode', 'RemainingStock', 'AvgPurchasePrice', 'PurchasePrice'];
  ...
  constructor(private apiService: vaplongapi,private storageService: StorageService, private datepipe: DatePipe, private notificationService: NotificationService) {
    ...
    const obj = {
      Action: 'View',
      Description: `View Stock Report with Prices`,
  ...
  GetAllStockDataWithLazyLoadinFunction(filterRM) {
    ...
    if (this.stockRequestModel.ClassificationID === -1 && this.stockRequestModel.DepartmentID === -1 &&
      ...
      this.apiService.GetProductStockListTotalCount(param).pipe(untilDestroyed(this)).subscribe((response: any) => {
      ...
      this.apiService.GetAllProductStockOverall(param).pipe(untilDestroyed(this)).subscribe((response1: any) => {
