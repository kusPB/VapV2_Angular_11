...
<div *ngIf="root" class="layout-menuitem-root-text" >{{item.label}}</div>
        ...
        <a (click)="itemClick($event)"  (mouseenter)="onMouseEnter()" *ngIf="item.routerLink && !item.items"
            ...
            [@children]="app.isSlim() ? (root ? app.isMobile()? 'visible':
			...
			(active ? 'visible' : 'hidden')) : (root ? 'visible' :(active ? 'visibleAnimated' : 'hiddenAnimated'))">
				<ng-template ngFor let-child let-i="index" [ngForOf]="item.items">
					<li *ngxPermissionsOnly="child.id" app-menuitem [item]="child" [index]="i" [parentKey]="key" [class]="child.badgeClass"></li>
				</ng-template>
			</ul>
...
export class AppMenuitemComponent implements OnInit, OnDestroy {
    ...
    constructor(public app: ShellComponent, public router: Router, private cd: ChangeDetectorRef, private menuService: MenuService) {
