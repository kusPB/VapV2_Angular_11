...
export class DashboardNewComponent implements OnInit, OnDestroy {
  ...
  isCustomDate = false;
  dateId = 0;
  constructor(private apiService: vaplongapi, private notificationService: NotificationService, private datepipe: DatePipe
  ) {
  ...
  ngOnInit(): void {
   ...
   //  -1, -1, -1, -1, -1, -1, -1, -1, new Date(), new Date(), 1000, 0, true, true,
   //  -1, -1, -1, true, true, true, '', '', true, true, -1, -1, true, true, '', ''
   //);
  ...
  GetTodayHeadQuarterDashBoardStatsCount() {
    ...
    this.apiService.GetTodayHeadQuarterDashBoardStatsCount(this.filterRequestModel).
  ...
  GetTopProductBySaleForDashboard() {
    if (this.dateId !== 7) {
      ...
      this.filterRequestModel1.ToDate = new Date(this.datepipe.transform(daterequest.ToDate, 'yyyy-MM-ddTHH:mm:ss'));
      this.filterRequestModel1.FromDate = new Date(this.datepipe.transform(daterequest.FromDate, 'yyyy-MM-ddTHH:mm:ss'));
    }
    else {
      this.filterRequestModel1.IsGetAll = false;
      this.filterRequestModel1.ToDate = new Date(this.datepipe.transform(this.toDate, 'yyyy-MM-ddTHH:mm:ss'));
      this.filterRequestModel1.FromDate = new Date(this.datepipe.transform(this.fromDate, 'yyyy-MM-ddTHH:mm:ss'));
    }
    // this.filterRequestModel.FromDate = new Date(new Date().setMonth(new Date().getMonth() - 11));
    this.apiService.GetTopProductBySaleForDashboard(this.filterRequestModel1).pipe(untilDestroyed(this)).subscribe((response: any) => {
      if (response.ResponseCode === 0) {
  ...
  GetTopProductByPurchaseForDashboard() {
    // this.filterRequestModel.FromDate = new Date(new Date().setMonth(new Date().getMonth() - 11));
    this.apiService.GetTopProductByPurchaseForDashboard(this.filterRequestModel1).pipe(untilDestroyed(this)).subscribe((response: any) => {
      if (response.ResponseCode === 0) {
  ...
  GetAnnualDashboardStatsForSale() {
    ...
    this.apiService.GetAnnualDashboardStatsForSale(this.filterRequestModel).pipe(untilDestroyed(this)).subscribe((response: any) => {
  ...
  GetAnnualDashboardStatsForPurchase() {
    ...
    this.apiService.GetAnnualDashboardStatsForPurchase(this.filterRequestModel).pipe(untilDestroyed(this)).subscribe((response: any) => {
  ...
  selectLineSaleChartdata(event) {
    ...
    this.notificationService.notify(NotificationEnum.INFO, this.lineSaleChartdata.datasets[event.element._datasetIndex].label, this.lineSaleChartdata.datasets[event.element._datasetIndex].data[event.element._index]);
  }
  selectPieSaleChartData(event) {
    // tslint:disable-next-line: max-line-length
    this.notificationService.notify(NotificationEnum.INFO, this.pieSaleChartData.labels[event.element._index], this.pieSaleChartData.datasets[event.element._datasetIndex].data[event.element._index]);
  }
  selectLinePurchaseChartdata(event) {
    // tslint:disable-next-line: max-line-length
    this.notificationService.notify(NotificationEnum.INFO, this.linePurchaseChartdata.datasets[event.element._datasetIndex].label, this.linePurchaseChartdata.datasets[event.element._datasetIndex].data[event.element._index]);
  }
  selectPiePurchaseChartData(event) {
    // tslint:disable-next-line: max-line-length
    this.notificationService.notify(NotificationEnum.INFO, this.piePurchaseChartData.labels[event.element._index], this.piePurchaseChartData.datasets[event.element._datasetIndex].data[event.element._index]);
  }
