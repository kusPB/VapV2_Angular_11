...
export class RefundSaleDetailComponent implements OnInit, OnDestroy {
  ...
  usermodel: any;
  constructor(private apiService: vaplongapi, private notificationService: NotificationService,public route: Router,public router: ActivatedRoute, private storageService: StorageService )
  {
  ...
  PrintingReturnSaleFuntion(id) {
    ...
    this.apiService.GetReturnSaleByID(req).pipe(untilDestroyed(this)).subscribe((response1: any) => {
      if (response1.ResponseCode === 0) {
        ...
        this.SaleDetails.txtSubTotal = response1.ReturnSale.ReturnSaleDetails.reduce((sum, current) => sum + current.dTotalValue, 0);
        this.SaleDetails.txtTotalDiscount = response1.ReturnSale.ReturnSaleDetails.reduce((sum, current) =>
