...
@Component({
  selector: 'app-hold-sale-detail',
  templateUrl: './hold-sale-detail.component.html',
  styleUrls: ['./hold-sale-detail.component.scss']
})
export class HoldSaleDetailComponent implements OnInit, OnDestroy {
  ...
  constructor(
    private apiService: vaplongapi, private notificationService: NotificationService, private router: Router,
...
<body onload='window.print();self.close();'>${printContents}</body>
  ...
  PrintingInvoiceFuntion(id) {
    const req = { ID: id };
    this.apiService.GetOpenSalesByID(req).pipe(untilDestroyed(this)).subscribe((response: any) => {
      if (response.ResponseCode === 0) {

        this.SaleDetails = response.OpenSale;
        const saleDetails = [];
        ...
        this.SaleDetails.newSaleDetails = [...this.SaleDetails.OpenSaleDetails];
        this.SaleDetails.OpenSaleDetails.forEach(item => {
