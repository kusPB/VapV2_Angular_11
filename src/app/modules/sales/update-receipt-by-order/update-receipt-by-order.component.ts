...
@Component({
  selector: 'app-update-receipt-by-order',
  templateUrl: './update-receipt-by-order.component.html',
  styleUrls: ['./update-receipt-by-order.component.scss'],
  providers: [DatePipe, ConfirmationService],
})
export class UpdateReceiptByOrderComponent implements OnInit, OnDestroy {
  cartProducts: any[] = [];
    ...
    this.apiService.GetPerformaSalesByID(param).pipe(untilDestroyed(this)).subscribe((response: any) => {
      if (response.ResponseCode === 0) {
        ...
        if (response.PerfomaSale.PaymentConditionID != null) {
          const paymentcondition = this.paymentConditionDropdown.filter(x => x.value === response.PerfomaSale.PaymentConditionID).shift();
        ...
        const deliveryMethod1 = this.AllDeliverMethodList.find(
        ...
        for (const item of response.PerfomaSale.PerfomaSaleDetails) {
          ...
          if (prodVarIDs.filter(x => x === item.ProductVariantID).length > 0) {
            continue;
          }
          const newTrackableList: TrackableProductsSaleDetailModel[] = [];
          // let selectedproduct;         
          // if (this.AllProductListwithDiscountForSelection.length > 0) {
          //   selectedproduct = this.AllProductListwithDiscountForSelection.filter(x =>
          ...
          this.apiService.GetProductByID(params).pipe(untilDestroyed(this)).subscribe((response1: any) => {
            if (response1.ResponseCode === 0) {
              ...
              prod.ProdMinDisc = 0,
                ...
                prod.QuantityLimitExceeds = (prod.Quantity>prod.OriginalStock)?true:false,
      ...
      if (response.ResponseCode === 0) {
        ...
        if (response.OpenSale.ID === 0) {
          ...
          if (response.OpenSale.PaymentConditionID != null) {
            const paymentcondition = this.paymentConditionDropdown.filter(x => x.value === response.OpenSale.PaymentConditionID).shift();
            this.selectedPaymentCondition = {
              value: paymentcondition.value,
              label: paymentcondition.label,
            };
            ...
            //this.apiService.GetProductByProductID(params).pipe(untilDestroyed(this)).subscribe((response1: any) => {
              ...
              if (response1.ResponseCode === 0) {
                ...
                if (item.TrackableProductsOpenSaleDetails.length > 0) {
                  item.TrackableProductsOpenSaleDetails.forEach(element => {
                    const trackableitem =
                    {
    ...
    this.apiService.GetAllCustomer().pipe(untilDestroyed(this)).subscribe((response: any) => {
      if (response.ResponseText === 'success') {
        ...
        if (this.activatedRoute.snapshot.params.id) {
          ...
          this.getPerformaSaleDetailsById(this.activatedRoute.snapshot.params.id);
        }
        ...
        this.AllProductLocationList.forEach(item => {
          if (selectedRow.IsAdded) {
            if (this.isLocationChangedByField) {
              ...
              locations = this.cartProducts.filter(x => x.ProductVariantID === item.ProductVariantID).shift();
          ...
          if (autoAssignLocationWhenProduct) {
            ...
            if (selectedRow.IsAdded) {
              ...
              loc = this.productLocations.filter(x => x.NonTrackableProductsLocationID === location1.NonTrackableProductsLocationID &&
  ...
  onRemoveSelectedTrackable(newValue: any) {
    ...
    if (this.cartProducts.length > 0) {
      ...
      if (!(selectedtrackables + '' === 'undefined' || selectedtrackables + '' == undefined || selectedtrackables === null)) {
        ...
        if (removed) {
          // let detailsIndex = this.cartProducts.findIndex(x => x.ProductVariantID === this.selectedRowForDialogs.ProductVariantID &&
          // x.ProductID === this.selectedRowForDialogs.ProductID && x.CartDetailID === this.selectedRowForDialogs.CartDetailID);
...
<body >${printContents}</body>
  ...
  Close() {
    this.router.navigate(['/order/manage-incoming-order']);
  }
