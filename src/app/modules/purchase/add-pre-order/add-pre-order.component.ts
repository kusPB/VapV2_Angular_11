...
@Component({
  selector: 'app-add-pre-order',
  templateUrl: './add-pre-order.component.html',
  styleUrls: ['./add-pre-order.component.scss'],
  providers: [DatePipe]
})

export class AddPreOrderComponent implements OnInit, OnDestroy {
  ...
  columns: Columns[] = [
    ...
    {
      field: 'CurrentBalance', header: 'CurrentBalance', sorting: 'CurrentBalance',permission:PurchasePermissionEnum.CurrentBalanceColumninPre_OrderCreate, placeholder: '',
 ...
 if(!this.isCapchaValidated)
 {
  ...
  AddOpenPurchaseApiCall(data) {
    ...
    this.apiService.AddOpenPurchase(obj).pipe(untilDestroyed(this)).subscribe((response: any) => {
      if (response.ResponseCode === 0) {
        this.notificationService.notify(NotificationEnum.SUCCESS, 'Success', 'Pre Order Added Successfull');
        this.router.navigate(['/purchase/pre-order-report']);
